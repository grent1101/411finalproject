...
int main(int argc, char *argv[]) {
  ...
  state.IFID.instr = state.IDEX.instr = state.EXMEM.instr = state.MEMWB.instr = state.WBEND.instr = NOPINSTRUCTION;
   /* nop */
...
void run(Pstate state) {
  ...
  while (1) {
    ...
    if ((opcode(instr) == REG_REG_OP) && (opcode(state->IDEX.instr) == LW_OP)) {
      if ((field_r1(instr) == field_r2(state->IDEX.instr)) || (field_r2(instr) == field_r2(state->IDEX.instr))) {
    ...
    } else if ((opcode(instr) != HALT_OP) && (opcode(state->IDEX.instr) == LW_OP)) {
    ...
    } else if (opPrev3 == LW_OP || (opPrev3 == ADDI_OP) || opPrev3 == BEQZ_OP) {
    ...
    } else if (opPrev2 == LW_OP || opPrev2 == ADDI_OP || opPrev2 == BEQZ_OP) {
    ...
    } else if (opPrev1 == LW_OP || opPrev1 == ADDI_OP || opPrev1 == BEQZ_OP) {
